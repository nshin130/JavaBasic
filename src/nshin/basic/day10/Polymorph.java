...
public class Polymorph {
    public static void main(String[] args) {
        ...
        // paint메서드로 구현해서 draw호출을 수월하게 바꿔봄 (2)
        // 여전히 draw메서드 호출시 동일한 클래스 타입을 사용해야 함
        // 새로운 도형이 추가되면 paint method도 새로 추가해야 함
        paint(new Line());
        paint(new Circle());
        paint(new Triangle());
        ...
        //이렇게 해도 되지만 for문을 이용해 반복하면 더 용이
//        shapes[0].draw();
//        shapes[1].draw();
//        shapes[2].draw();
        ...
        // 바인딩에는 정적바인딩/동적바인딩이 존재
        // 정적바인딩 : 커파일시 변수의 타입이나 함수의 위치가 정해짐
        // 실행전에 많은 정보들이 미리 결정되기 때문에 실행효율 증가

        // 동적바인딩 : 실행시 변수의 타입이나 함수의 위치가 결정
        // 실행시 자유롭게 변수의 성격이 바뀌므로 적응성 증가

    }

    //(2) paint method 자식 클래스 사용

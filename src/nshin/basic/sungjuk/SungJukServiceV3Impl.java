...
public class SungJukServiceV3Impl implements SungJukServiceV3 {
    ...
    // 멤버변수 선언(메서드 밖에서 선언된 변수 <-> 지역변수 local variable)
    // 입력받은 모든 성적데이터를 저장하는 동적배열 변수 선언
    List<SungJukVO> sjdata = new ArrayList<>();
    SungJukVO sj = null;
    // 성적데이터를 저장해 둘 파일경로 지정
    String fpath = "/Users/nara/01JAVAb/SungJuk.data";
    String fpath2 = "/Users/nara/01JAVAb/SungJuk.csv";
    String fpath3 = "/Users/nara/01JAVAb/SungJuk.json";

    // CSV 직렬화를 위한 문자열 포맷
    String csvfmt = "%s,%d,%d,%d,%d,%.1f,%s\n";

    // JSON 직렬화를 위한 문자열 포맷
    String jsonfmt = "{\"name\":\"%s\",\"kor\":%d,\"eng\":%d,\"math\":%d," +
                     "\"tot\":%d,\"avg\":%.1f,\"grd\":\"%s\"},\n";
    ...
    public void showOneSungJuk() {
            ...
            try {
                ...
                System.out.printf(fmt, pos,
                        sj.getName(), sj.getKor(), sj.getEng(), sj.getMath(),
    ...
    public void initSungJuk() {
        ...
        // finally 구문 : 예외처리가 발생했을때 객체를 닫는 작업이 실행되기 전 오류가 생기면
        //                객체를 닫는 작업이 실행되지 않으므로 finally 구문에 넣어 강제로 처리.
    }
    ...
    // 기존에 입력된 성적데이터를 csv 역직렬화를 통해
    // 파일에서 읽어서 메모리에 적재하는 기능담당
    @Override
